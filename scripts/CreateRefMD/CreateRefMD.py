...
# Usage: CreateRefMD.py [arguments]
...
class CreateReferencedMD(CBase):
                    ...
                    if not arcpy.Exists(mdPath):
                        try:
                            if len(self.dic_ref_info) > 0:
                                ...
                                arcpy.CreateReferencedMosaicDataset_management(
                                    inMosaic, refMosaic, self.srs, "", self.pixel_type, "", "", "", "", "", "", "", "", "NO_BOUNDARY"
                                )
                    ...
                    for fnc in self.dic_derive_lst[k]["ref"][r]:
                        ...
                        try:
                            arcpy.EditRasterFunction_management(
                                refMosaic, "EDIT_MOSAIC_DATASET", "REPLACE" if first_time else "INSERT", fnc
                            )
                            first_time = False
                        except BaseException:
    ...
    def setMDProperties(self, config):
        setMDProps = SetMDProperties(self.m_base)
    ...
    def init(self, config):
        ...
        if Nodelist.length == 0:
            self.log("MosaicDatasets node not found! Invalid schema.", self.const_critical_text)
            return False
        ...
        try:
                ...
                if node is not None and node.nodeType == self.m_base.NODE_TYPE_ELEMENT:
                    if node.nodeName == "CreateReferencedMosaicDataset":
                        ...
                        if len(self.dic_derive_lst) > 0:  # if CreateReferencedMosaicDataset is found, addRaster info gets ignored.
                            continue
                        if len(refMD) == 0:
                            self.log("<MosaicDataset/Name> should be defined first.", self.const_critical_text)
                            return False
                        for node in node.childNodes:
                            if node is not None and node.nodeType == self.m_base.NODE_TYPE_ELEMENT:
                                nodeName = node.nodeName.lower()
            ...
            self.log(f"Reading MosaicDataset nodes.\n{exp}", self.const_critical_text)
            return False
        return True
