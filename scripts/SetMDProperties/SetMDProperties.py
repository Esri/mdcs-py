...
# Usage: SetMDProperties.py [arguments]
...
class SetMDProperties(CBase):
        ...
        try:
            ...
            for k in set(common_keys):
                if isinstance(first_dict[k], dict):
                    z0 = self.compare_dict(first_dict[k], second_dict[k], outputJson)
                else:
                ...
                if not z0_bool:
                    ...
                    message = (
                        "Property:" + str(k) + " --->>> First Mosaic:" + str(first_dict[k]) + "  |||  Second Mosaic:" + str(second_dict[k])
                    )
    ...
    def extractPropertytoJson(self, mdObj, outputJson):
        ...
        try:
            ...
            log.Message(f"{exp}", self.const_critical_text)
            return False
    ...
    def comparePropertyByMosiac(self, internal_mosaic, external_mosaic, outputJson):
        ...
        log = self.m_base.m_log
        firstMosaicProperty = secondMosiacProperty = {}
        try:
    ...
    def setProperty(self, mdPath):
        ...
        setProperties = MBase.DynaInvoke("arcpy.SetMosaicDatasetProperties_management", args, log=self._message)
    ...
    def setMDProperties(self, mdPath):
        ...
        try:
            if sp_flag == "setpropertybyjson":
                ...
                if sp_outputjson == "#":
                    log.Message("Missing Output file to Extract property", self.const_critical_text)
                    log.Message("Extracting property to Parameter/Json/", self.const_critical_text)
                    sp_outputjson = os.path.join(
                        jsonPath, "mosaicProperty" + str(datetime.strftime(datetime.now(), "%Y%m%d%H%M%S")) + ".json"
                    )
                ...
                if sp_mosaic == "#":
                    if sp_inputjson == "#":
                        ...
                        if sp_outputjson == "#":
                            ...
                            log.Message("Extracting property to Parameter/Json/", self.const_critical_text)
                            sp_outputjson = os.path.join(
                                jsonPath, "compare" + str(datetime.strftime(datetime.now(), "%Y%m%d%H%M%S")) + ".json"
                            )
                            absPathCheck = os.path.isabs(sp_inputjson)
                            if not absPathCheck:
                                sp_inputjson = os.path.join(jsonPath, sp_inputjson)
    ...
    def init(self, config):
        ...
        if Nodelist.length == 0:
            self.log("MosaicDataset node not found! Invalid schema.", self.const_critical_text)
            return False
        try:
                                        ...
                                        for each in ptvaluesplit:
                                            if each.find("/") == -1:
                                                if each.lower() == "none":  # 'none' is an acceptable value.
                                                    ...
                                                    rftpaths = rftpaths + os.path.abspath(
                                                        os.path.join((self.m_base.const_raster_function_templates_path_), each)
                                                    )
                                                rftpaths += ";"
                                        ptvalue = rftpaths = rftpaths[:-1]
                                self.dic_properties_lst[node.nodeName] = ptvalue
        except BaseException as exp:
            self.log(f"{exp}", self.const_warning_text)
        return True
