...
# Usage: ProcessInfo.py [arguments]
...
class ProcessInfo(CBase):

    def __init__(self, base=None):
        self.m_hsh_parent_child_nodes = {
            "addindex": {"parent": "addindex", "child": "index"},
            "calculatevalues": {"parent": "calculatevalues", "child": "calculatevalue"},
        }
        self.m_base = base
        self.setLog(base.m_log)

    def getXML(self):
        if self.m_base.m_doc is not None and self.config != "":
    ...
    def init(self, config):
        ...
        try:
            ...
            self.log("Error: Reading <MosaicDataset> node.", self.const_critical_text)
            return False
