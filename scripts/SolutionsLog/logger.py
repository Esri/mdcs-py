...
# Usage: logger.py [arguments]
# Author:  Esri Imagery Workflows team
# -------------------------------------------------------------------------------
# !/usr/bin/env python

from xml.dom.minidom import Document  # nosec - verified (ref. chamlika)
...
class Logger(object):
    ...
    def Message(self, message, messageType):
        ...
        key = self.active_key
        errorTypeText = "msg"
        if messageType is None or messageType == self.const_general_text or messageType == self.const_status_text:
        ...
        if self.isGPRun:
            ...
            if self.m_base:
                if hasattr(self.m_base, "invoke_cli_msg_callback"):  # used by MDCS
                    msg_codes = ["INF", "WRN", "ERR", "STS", "CMD"]
                    msg_type = msg_codes[self.const_general_text]  # default
                    if messageType >= self.const_general_text and messageType <= self.const_status_text:
                        msg_type = msg_codes[messageType]
    ...
    def make_log_filename(self):
        ...
        if prefix == "":
            prefix = "log"
        record_updated = prefix + "_%04d%02d%02dT%02d%02d%02d.xml" % (
            ousr_date.year,
            ousr_date.month,
            ousr_date.day,
            ousr_date.hour,
    ...
    def is_log_CSV(self):
        ...
        if not self.logFileName:
            return False
        if self.logFileName[-4:].lower() == ".csv":
            return True
        return False